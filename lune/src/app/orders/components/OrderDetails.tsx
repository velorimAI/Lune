  ...
  <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-md mt-2">
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm text-right text-gray-800">
        ...
        <tbody>
          {order?.receptions?.map((reception: { reception_number: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined; orders: any[]; }, i: React.Key | null | undefined) => (
            <React.Fragment key={i}>
              ...
              {reception?.orders?.map((part, j) => (
                <tr key={j} className={`border-b ${j % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
                  ...
                  <td className="px-4 py-3">{part?.order_date?.split(" ")[0]}</td>
